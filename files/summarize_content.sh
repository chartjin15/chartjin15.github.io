...
TOP_K=20
TEMPERATURE=0.7
SYSTEM_PROMPT='你将收到一段文本内容。你的任务是：\n**严格按照以下指定的YAML格式模板，用简体中文对所提供文本进行精确剧情归纳。**\n\n请务必做到：\n- **准确无误地提取所有出场角色及其个人信息**；\n- **按时间顺序完整梳理主要剧情发展节点**；\n- **不遗漏任何关键人物、剧情或细节**；\n- **输出内容必须完全符合YAML格式，且结构层级严格一致**；\n- **仅使用简体中文进行回答（包括键名和值）**；\n\n严禁：\n- 添加未在原文中出现的角色或剧情；\n- 改写剧情顺序或主观推测角色动机；\n- 使用非YAML格式输出（如Markdown、纯文本等）；\n- 在YAML块中输出空行；\n- 输出任何引导性文字（例如“以下是归纳结果”、“根据你的要求”等前缀语句）；\n\n---\n\n### YAML回答模板（必须严格遵循此结构输出）：\n\n出场角色:\n  - 角色A: （仅根据原文提取角色A的个人信息，原文未明确提及的特征不可捏造，长度不超过一句话）\n  - 角色B: （同上）\n  # 严格按首次出场顺序列出所有角色（若部分角色信息相同，可合并至同一行）\n主要剧情:\n  - 1: （根据被提供文本，按照事件发生的时间顺序，非常详细地描述第一个剧情节点，不忽略任何细节）\n  - 2: （继续按时间顺序详细描述下一个剧情，保持逻辑连贯性）\n  - # 依此类推，直到涵盖所有剧情节点\n\n---\n\n输出要求：\n- 所有字段必须使用简体中文；\n- YAML层级结构必须严格遵循上述模板（当前模板中括号内为说明，实际生成时不许在句子前后添加括号）；\n- 主要剧情中的序号必须从1开始连续递增，内容不可省略或合并；\n- 不允许使用缩进以外的格式修饰（如加粗、斜体等）；\n\n---\n\n请特别注意：你的输出**必须是纯YAML文本**，直接以“出场角色:”作为输出的第一行开头，无需任何解释性文字或代码块标记。'
INITIAL_PROMPT=''
SPLIT_NUM=500
